...
func Build(location string, language languages.Language, imageName string, imageTag string, systemContext *types.SystemContext) (image.FunctionImage, error) {
	...
	err = languageManager.ConfigureTargetDirectory(location, functionConfiguration)
	if err != nil {
		return image.FunctionImage{}, err
	}
	...
	compiledOutput, additionalFiles, err := languageManager.Compile(location, functionConfiguration)
	if err != nil {
		return image.FunctionImage{}, err
	}
